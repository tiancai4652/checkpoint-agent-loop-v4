...
v4 = v3 全部硬规则（大白话汇报、后台任务监控、轮次归档）+ **设计链路（自动分级）** + **PM 增强（PM Skills 按需 + 定稿前 RedTeam 批判）**。
...
3. **有没有坏消息**：问题/风险/后台任务健康状况（见下「后台长任务」）。没有就明说"没有坏消息"，不许藏着掖着，也不许制造焦虑。
...
```
命中任一即视为已装（直接用）；都没有才自装，**自装前先 `mkdir -p ~/.config/opencode/skills`**（新设备可能没有这个根），**安装目标统一为 `~/.config/opencode/skills/`**。
...
4. **硬性关卡不适用**：初始化确认关卡（PRD 未确认）、设计链路 L2 首次拍板（含 huashu 高保真选型）绝不超时默认，必须等到用户明确拍板。
...
用 loop 脚本反复调 `opencode run --session <id> --continue --agent engineer "继续执行 PRD.md 当前任务"`（PRD.md 是根指针，指向本轮 run 目录），并 grep 输出签名决定是否 break。脚本处理 `[[CHECKPOINT]]`（等评审/超时默认）、`[[NEEDS-USER]]`（停）、`[[NEED-RESEARCH]]`（停，无人值守无法派研究员）；有 TaskDeck 就用 task-run 派发它（面板可见、可判健康），没有则 nohup + 日志。
