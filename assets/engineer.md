...
v2 硬规则一：后台长任务走 TaskDeck：
- 预计运行超 1 分钟、且要在后台跑的命令/脚本，**禁止裸 `nohup`/`&`**，必须用 `python3 ~/tools/taskdeck/task-run.py --name "短标题" --goal "大白话说明做什么、预计多久、产出什么" [--max-minutes N] -- <命令>` 派发。
- `--goal` 必须用大白话写（用户在面板上靠它理解）。派发后把详情页 URL 报给驾驶者转达用户。
...
- **探活 → 缺则自装 → 装不了降级**（全部非硬依赖，来源/命令见 DRIVER 依赖矩阵）：`ui-ux-pro-max` / `Webdesign` / `huashu-design`（自装剔除 BGM/demos）；缺失时降级（用常识给口语化风格候选 / 自行按方向写 / 跳过原型），不阻塞、不编造。
