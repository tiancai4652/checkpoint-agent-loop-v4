...
- **不可用（换台设备/没装）** → 先自装：`git clone --depth 1 https://github.com/tiancai4652/taskdeck /tmp/taskdeck && bash /tmp/taskdeck/install.sh && rm -rf /tmp/taskdeck`（装完再探活，可用即用 task-run）；自装失败（无网络等）→ 回退 `nohup <命令> > docs/runs/NNN-<功能名>/logs/<名字>.log 2>&1 &`（记录 pid），把日志路径告诉用户；**不得卡住、不得静默裸跑**。
